...
class Controller:
    def __init__(self):
        ...
        self.target_face = "face"                # 人脸YOLO标签（替换原始t_0）
        self.target_gesture_right = "gesture_3"  # 右手3完整标签（替换原始target_number_right=3）
        self.target_gesture_both = "gesture_5"   # 双手5完整标签（替换原始target_number_both=5）

        self.target_center_offset = 35  # 目标中心与屏幕中心偏移量，单位像素

        # YOLO 目标参数
         # ========== YOLO模型配置（扩展多模型，仍用单API承载） ==========
        self.yolo_model_vehicle = YoloModel.VEHICLE
        self.yolo_model_face = YoloModel.FACE
        self.yolo_model_gesture = YoloModel.GESTURE
        self.current_yolo_model = None  # 记录当前激活的YOLO模型（新增，用于单API切换）
        ...
        # 传感器和执行器
   # ========== 传感器和执行器（与原文件一致，仅用1个API实例） ==========
        self.api = UpAPI(grayscale_threshold=self.grayscale_threshold)  # 单API实例，不新增多实例
            ...
            if self.state_main == MainState.IDLE:
                ...
                if self.state_recognition == RecognitionState.TURN_LEFT:
                    ...
                    if self.state_target == TargetState.APRIL_TAG:
                        ...
                        # ========== 【修改】手势改为YOLO预加载（替换原detect_gesture） ==========
                        print("手势YOLO识别区域，预加载图像中")
                        # 单API切换为手势YOLO模型（内部动态加载）
                        try:
                            self.api.switch_yolo_model(self.yolo_model_gesture)
                        except Exception as e:
                            print(f"切换手势YOLO模型失败: {e}")
                        preload_complete = self.api.preload_yolo_pool()
                        if preload_complete:
                            print("手势YOLO预加载完成，准备瞄准")
                            self.timer_arm_action.start()
                            self.state_main = MainState.RECOGNITION
...
# ========== 【修改】人脸改为YOLO预加载（替换原detect_face） ==========
                        print("人脸YOLO识别区域，预加载图像中")
                        try:
                           self.api.switch_yolo_model(self.yolo_model_face)
                        except Exception as e:
                               print(f"切换人脸YOLO模型失败: {e}")
                        preload_complete = self.api.preload_yolo_pool()
                        if preload_complete:
                            print("人脸YOLO预加载完成，准备瞄准")
                            self.state_main = MainState.RECOGNITION
                            self.state_recognition = RecognitionState.AIM
                    ...
                    if self.state_target == TargetState.YOLO:
                        ...
                        if find_target:
                         ...
                         # ========== 【修改】人脸用YOLO检测（替换原detect_face） ==========
                        print("人脸YOLO瞄准中")
                        find_target, offset_x = self.api.detect_yolo(label=self.target_face)
                        if find_target:
                            self.__aim_target(offset_x)
                    elif self.state_target == TargetState.GESTURE:
                        # ========== 【修改】手势用YOLO检测（替换原detect_gesture） ==========
                        print("手势YOLO瞄准中")
                        # 检测两个手势标签，标记类型并计算偏移
                        find_right, offset_right = self.api.detect_yolo(label=self.target_gesture_right)
                        find_both, offset_both = self.api.detect_yolo(label=self.target_gesture_both)
                        if find_right:
                           self.current_gesture_type = "right"
                           self.__aim_target(offset_right)
                        elif find_both:
                          self.current_gesture_type = "both"
                          self.__aim_target(offset_both)
                    ...
                    if self.state_target == TargetState.APRIL_TAG:
                        ...
                        if self.timer_arm_action.complete():
                            ...
                            print("做手势动作中")
                            if self.current_gesture_type == "right":
                            ...
                            elif self.current_gesture_type == "both":
                        ...
                        # ========== 【修改】人脸用YOLO检测结果执行击打（保留原击打动作） ==========
                        find_target, _ = self.api.detect_yolo(label=self.target_face)
                        if find_target:
                            print(f"开始击打人脸YOLO目标：{self.target_face}")
                            self.__hit_actions()
                            self.state_main = MainState.TRANSITION
                            self.state_transition = TransitionState.SPAN
                            self.state_span = SpanState.RIGHT
